...
<?php elseif($version ?? false): ?>
    <a href="<?= esc($link ?? '#') ?>" class="px-12 py-5 rounded-lg font-bold text-2xl text-white color-dark-espresso hover-primary cursor-pointer">
        <?= esc($btnName ?? 'Action')?>
    </a>
<?php else: ?>
    <a href="<?= esc($link ?? '#') ?>" class="px-6 py-2 rounded-lg font-bold text-white color-dark-espresso hover-primary cursor-pointer">
        <?= esc($btnName ?? 'Action')?>
    </a>
<?php endif; ?>