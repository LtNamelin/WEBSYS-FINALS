        ...
        <main class="space-y-12 py-10">
            <div class="md:flex md:space-x-6">
                ...
                <section class=" mx-8 my-8 p-6 color-light-cappuccino rounded-xl shadow">
                    ...
                    <form method="post" action="/admin/menuPage" enctype="multipart/form-data" class="mb-8 space-y-3">
                        <div class="flex flex-col sm:flex-row gap-3">
                            <input type="text" name="product_name" placeholder="Product Name" class="shadow-sm px-3 py-2 color-light-latte border border-slate-200 rounded-md w-full sm:w-1/3" required>
                            ...
                            <input type="text" name="product_description" placeholder="Description" class="shadow-sm px-3 py-2 color-light-latte border border-slate-200 rounded-md w-full sm:w-1/3" required>
                            <input type="text" name="type" placeholder="Coffee" class="shadow-sm px-3 py-2 color-light-latte border border-slate-200 rounded-md w-full sm:w-1/3" required>
                            <input type="file" name="product_image" accept="image/*" class="shadow-sm px-3 py-2 color-light-latte border border-slate-200 rounded-md w-full sm:w-1/4" required>
                                        ...
                                        <tr>
                                            ...
                                            <td class="px-6 py-4">
                                                <?php if (!empty($product->product_image)): ?>
                                                    <img src= "/assets/uploads/images/products/<?= esc($product->product_image) ?>" alt="Product Image" class="w-16 h-16 object-cover rounded">
                                                <?php else: ?>
                                                    <span class="text-gray-400 italic">No image</span>
                                                <?php endif; ?>
                                            </td>
                                            <td class="px-6 py-4 text-center">
                                                <form method="post" action="/admin/menuPage" onsubmit="return confirm('Delete this product?');">
                                                    <button type="submit"  name="delete" value="<?= esc($product->id) ?>" class="bg-red-600 hover:bg-red-700 text-white px-3 py-1 rounded-md">Delete</button>
                                                </form>
                                            </td>
                                        </tr>
                                    <?php endforeach; ?>
